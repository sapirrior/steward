  ...
  summarize: (args, result) => {
    ...
    for (const hunk of diff.hunks) {
      for (const line of hunk.lines) {
        if (line.kind === 'addition') diffAdded++;
        if (line.kind === 'deletion') diffRemoved++;
      }
    }

    const added =
      result?.addedLines !== undefined && result.addedLines > 0 ? result.addedLines : diffAdded;
    const removed =
      result?.removedLines !== undefined && result.removedLines > 0
        ? result.removedLines
        : diffRemoved;
